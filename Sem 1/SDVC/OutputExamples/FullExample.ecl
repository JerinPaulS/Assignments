...
findSolutions(Instances):-
	...
	liveliness(CardVariables, "Paper"),
	...
	% General Constraints
	noStudentReviewers(Instances),
	noSelfReviews(Instances),
	paperLength(Instances),
	limitsOnStudentPapers(Instances),
...
roleMin("Reviews", "submission", 1).
roleMin("Reviews", "referee", 3).
roleMin("Writes", "manuscript", 1).
roleMin("Writes", "author", 1).
...
weakSatisfiability(CardVariables):- weakSatisfiabilityConstraint(CardVariables).

liveliness(CardVariables, "paper"):- livelinessConstraint(CardVariables, "paper").
...
creationPaper(Instances, Size, Attributes):-
	length(Instances, Size),
	(foreach(Xi, Instances), fromto([],AtIn,AtOut,Attributes), param(Size) do
		Xi=paper{oid:Integer1,wordCount:Integer2,studentPaper:Boolean3}, Integer1::1..Size, Integer2::0..10, Boolean3::0..1, 
		append([Integer1,Integer2,Boolean3],AtIn, AtOut)).
...
nallInstances1NoStudentReviewers( Instances,_, Result):-
	ocl_allInstances(Instances, "Paper", Result).

nVariable2_1_1NoStudentReviewers( _, Vars, Result):-
	ocl_variable(Vars,1,Result).

nNavigation2_1NoStudentReviewers( Instances, Vars, Result):-
	nVariable2_1_1NoStudentReviewers(Instances, Vars, Value1),
	ocl_navigation(Instances, "Reviews", "submission", "referee", Value1, Result).

nVariable2_2_1_1NoStudentReviewers( _, Vars, Result):-
	ocl_variable(Vars,1,Result).

nAttribute2_2_1NoStudentReviewers( Instances, Vars, Result):-
	nVariable2_2_1_1NoStudentReviewers(_, Vars, Object),
	ocl_attributeCall(Intances, "Researcher", "isStudent", Object, Result).

nnot2_2NoStudentReviewers( Instances, Vars, Result):-
	ocl_not(Instances, Vars, nAttribute2_2_1NoStudentReviewers, Result).

nforAll2NoStudentReviewers( Instances, Vars, Result):-
	nNavigation2_1NoStudentReviewers(Instances, Vars, Value1),
	ocl_col_forAll(Instances, Vars, Value1, nnot2_2NoStudentReviewers, Result).

nforAll0NoStudentReviewers( Instances, Vars, Result):-
	nallInstances1NoStudentReviewers(Instances, Vars, Value1),
	ocl_col_forAll(Instances, Vars, Value1, nforAll2NoStudentReviewers, Result).

noStudentReviewers(Instances):-
	nforAll0NoStudentReviewers(Instances, [], Result),
	Result #=1.


nallInstances1NoSelfReviews( Instances,_, Result):-
	ocl_allInstances(Instances, "Researcher", Result).

nVariable2_1_1_1NoSelfReviews( _, Vars, Result):-
	ocl_variable(Vars,1,Result).

nNavigation2_1_1NoSelfReviews( Instances, Vars, Result):-
	nVariable2_1_1_1NoSelfReviews(Instances, Vars, Value1),
	ocl_navigation(Instances, "Reviews", "referee", "submission", Value1, Result).

nasSet2_1NoSelfReviews( Instances,Vars, Result):-
	nNavigation2_1_1NoSelfReviews(Instances, Vars, Value1),
	ocl_set_asSet(Value1, Result).

nVariable2_2_1NoSelfReviews( _, Vars, Result):-
	ocl_variable(Vars,1,Result).

nNavigation2_2NoSelfReviews( Instances, Vars, Result):-
	nVariable2_2_1NoSelfReviews(Instances, Vars, Value1),
	ocl_navigation(Instances, "Writes", "author", "manuscript", Value1, Result).

nexcludes2NoSelfReviews( Instances, Vars, Result):-
	nasSet2_1NoSelfReviews(Instances, Vars, Value1),
	nNavigation2_2NoSelfReviews(Instances, Vars, Value2),
	ocl_set_excludes(Value1, Value2, Result).

nforAll0NoSelfReviews( Instances, Vars, Result):-
	nallInstances1NoSelfReviews(Instances, Vars, Value1),
	ocl_col_forAll(Instances, Vars, Value1, nexcludes2NoSelfReviews, Result).

noSelfReviews(Instances):-
	nforAll0NoSelfReviews(Instances, [], Result),
	Result #=1.


nallInstances1PaperLength( Instances,_, Result):-
	ocl_allInstances(Instances, "Paper", Result).

nVariable2_1_1PaperLength( _, Vars, Result):-
	ocl_variable(Vars,1,Result).

nAttribute2_1PaperLength( Instances, Vars, Result):-
	nVariable2_1_1PaperLength(_, Vars, Object),
	ocl_attributeCall(Intances, "Paper", "wordCount", Object, Result).

nConstant2_2PaperLength( _, _, Result):-
	Result=10000.

nless_than2PaperLength( Instances, Vars, Result):-
	ocl_int_less_than(Instances, Vars, nAttribute2_1PaperLength, nConstant2_2PaperLength, Result).

nforAll0PaperLength( Instances, Vars, Result):-
	nallInstances1PaperLength(Instances, Vars, Value1),
	ocl_col_forAll(Instances, Vars, Value1, nless_than2PaperLength, Result).

paperLength(Instances):-
	nforAll0PaperLength(Instances, [], Result),
	Result #=1.


nallInstances1_1LimitsOnStudentPapers( Instances,_, Result):-
	ocl_allInstances(Instances, "Paper", Result).

nVariable1_2_1LimitsOnStudentPapers( _, Vars, Result):-
	ocl_variable(Vars,1,Result).

nAttribute1_2LimitsOnStudentPapers( Instances, Vars, Result):-
	nVariable1_2_1LimitsOnStudentPapers(_, Vars, Object),
	ocl_attributeCall(Intances, "Paper", "studentPaper", Object, Result).

nexists1LimitsOnStudentPapers( Instances, Vars, Result):-
	nallInstances1_1LimitsOnStudentPapers(Instances, Vars, Value1),
	ocl_col_exists(Instances, Vars, Value1, nAttribute1_2LimitsOnStudentPapers, Result).

nallInstances2_1_1_1LimitsOnStudentPapers( Instances,_, Result):-
	ocl_allInstances(Instances, "Paper", Result).

nVariable2_1_1_2_1LimitsOnStudentPapers( _, Vars, Result):-
	ocl_variable(Vars,1,Result).

nAttribute2_1_1_2LimitsOnStudentPapers( Instances, Vars, Result):-
	nVariable2_1_1_2_1LimitsOnStudentPapers(_, Vars, Object),
	ocl_attributeCall(Intances, "Paper", "studentPaper", Object, Result).

nselect2_1_1LimitsOnStudentPapers( Instances, Vars, Result):-
	nallInstances2_1_1_1LimitsOnStudentPapers(Instances, Vars, Value1),
	ocl_set_select(Instances, Vars, Value1, nAttribute2_1_1_2LimitsOnStudentPapers, Result).

nsize2_1LimitsOnStudentPapers( Instances,Vars, Result):-
	nselect2_1_1LimitsOnStudentPapers(Instances, Vars, Value1),
	ocl_set_size(Value1, Result).

nConstant2_2LimitsOnStudentPapers( _, _, Result):-
	Result=5.

nless_than2LimitsOnStudentPapers( Instances, Vars, Result):-
	ocl_int_less_than(Instances, Vars, nsize2_1LimitsOnStudentPapers, nConstant2_2LimitsOnStudentPapers, Result).

nand0LimitsOnStudentPapers( Instances,Vars, Result):-
	ocl_and(Instances, Vars, nexists1LimitsOnStudentPapers, nless_than2LimitsOnStudentPapers, Result).

limitsOnStudentPapers(Instances):-
	nand0LimitsOnStudentPapers(Instances, [], Result),
	Result #=1.
